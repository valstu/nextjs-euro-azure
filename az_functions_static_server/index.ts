...
if (isOnAzure) {
  nextPath = path.dirname(
    require.resolve("../../basic-nextjs-example/.next/BUILD_ID")
  );
} else {
  nextPath = path.dirname(
    require.resolve("@nextjsonazure/basic-nextjs-example/.next/BUILD_ID")
  );
}
...
module.exports = async function (context: Context, req: HttpRequest) {
  ...
  try {
    const data = await fs.readFile(
      path.join(nextPath, "static", staticFilePath)
    );
